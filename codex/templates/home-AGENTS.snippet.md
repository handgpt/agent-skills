...
- If the Antigravity CLI log contains `E... not logged into Antigravity`, the wrapper prints matching `W...` and `E...` log lines and automatically relaunches `agy` up to 5 times by default. Override with `CODEX_AGY_AUTH_RETRIES` or `"auth_retries"` in `~/.codex/agy_cli.json`. If a skill command still exits with a login failure after those internal retries, Codex should rerun the same skill command up to 2 more times before treating Antigravity as unavailable.
...
- Gemini CLI advisory skills are removed from the Codex runtime because Gemini CLI is no longer supported. Migrate any `$gemini-*` workflow to `$agy-design-checkpoint`, `$agy-error-analysis`, or `$agy-review`.
